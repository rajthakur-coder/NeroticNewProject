...
import AnimatedDropdown from "../../../components/Common/AnimatedDropdown"; // ✅ IMPORTED AnimatedDropdown
import { motion } from "framer-motion"; // motion is still needed for the selected row overlay
...
const ProductPricing = () => {
  ...
  const [isDropdownOpen, setIsDropdownOpen] = useState(false); // ✅ RETAINED for AnimatedDropdown
  ...
  // ❌ REMOVED useEffect for handleClickOutside, as it's now inside AnimatedDropdown
  ...
  const actions = [
    { label: "Edit", icon: <EditIcon />, onClick: () => handleEdit(allOrders.find(o => o.id === selectedOrderId)) }, // Pass the correct data
    {
      label: "Delete",
      icon: <DeleteIcon />,
        ...
        <div className="p-4 sm:p-6">
          <div className="flex flex-col gap-4 mt-6 md:flex-row md:items-center md:gap-4">
            <div className="flex items-center w-full gap-2 md:flex-1">
              ...
              // height can be adjusted here if needed, e.g., height="40px"
              />
              {/* ================================== */}
