...
import PrintImportExportModal from "../components/Modal/PrintImportExportModal";
...
const ordersData: Order[] = [
    { id: "#6010", name: "Jayvion Simon", email: "nannie.abernathy70@yahoo.com", date: "20 Sep 2025", time: "11:01 am", items: 6, amount: "$484.15", status: "Refunded", avatar: "https://i.pravatar.cc/40?img=1" },
    { id: "#6011", name: "Lucian Obrien", email: "ashlynn.ohara62@gmail.com", date: "19 Sep 2025", time: "10:01 am", items: 1, amount: "$83.74", status: "Completed", avatar: "https://i.pravatar.cc/40?img=2" },
    { id: "#60110", name: "Soren Durham", email: "vergie.block82@hotmail.com", date: "10 Sep 2025", time: "1:01 am", items: 5, amount: "$400.41", status: "Pending", avatar: "https://i.pravatar.cc/40?img=3" },
    { id: "#60111", name: "Cortez Herring", email: "vito.hudson@hotmail.com", date: "09 Sep 2025", time: "12:01 am", items: 1, amount: "$83.74", status: "Completed", avatar: "https://i.pravatar.cc/40?img=4" },
    { id: "#60112", name: "Brycen Jimenez", email: "tyrel.greenholt@gmail.com", date: "07 Sep 2025", time: "11:01 pm", items: 6, amount: "$484.15", status: "Refunded", avatar: "https://i.pravatar.cc/40?img=5" },
];

const screenshotOrders: Order[] = [
    { id: "#6019", name: "Shawn Manning", email: "marjoline.white94@gmail.com", date: "16 Sep 2025", time: "1:20 am", items: 1, amount: "$83.74", status: "Completed", avatar: "https://i.pravatar.cc/40?img=21" },
    { id: "#6018", name: "Chase Day", email: "joana.simonis84@gmail.com", date: "17 Sep 2025", time: "2:20 am", items: 5, amount: "$400.41", status: "Pending", avatar: "https://i.pravatar.cc/40?img=22" },
    { id: "#6017", name: "Melanie Noble", email: "luella.ryan33@gmail.com", date: "18 Sep 2025", time: "3:20 am", items: 1, amount: "$83.74", status: "Completed", avatar: "https://i.pravatar.cc/40?img=23" },
    { id: "#6016", name: "Christopher Cardenas", email: "lenna.bergnaum27@hotmail.com", date: "19 Sep 2025", time: "4:20 am", items: 6, amount: "$484.15", status: "Cancelled", avatar: "https://i.pravatar.cc/40?img=24" },
    { id: "#6015", name: "Lainey Davidson", email: "aditya.greenfelder31@gmail.com", date: "20 Sep 2025", time: "5:20 am", items: 1, amount: "$83.74", status: "Completed", avatar: "https://i.pravatar.cc/40?img=25" },
    { id: "#6014", name: "Elias Graham", email: "elias.graham@gmail.com", date: "21 Sep 2025", time: "6:00 am", items: 2, amount: "$150.99", status: "Refunded", avatar: "https://i.pravatar.cc/40?img=26" },
];

const allOrders = [...screenshotOrders, ...ordersData];
const ROWS_PER_PAGE_OPTIONS = [5, 10, 25];

const orderTabs: Tab[] = [
    { name: "All", key: "All", count: allOrders.length },
    { name: "Pending", key: "Pending", count: allOrders.filter((o) => o.status === "Pending").length },
    { name: "Completed", key: "Completed", count: allOrders.filter((o) => o.status === "Completed").length },
    { name: "Cancelled", key: "Cancelled", count: allOrders.filter((o) => o.status === "Cancelled").length },
    { name: "Refunded", key: "Refunded", count: allOrders.filter((o) => o.status === "Refunded").length },
];
...
const OrderList = () => {
    ...
    const [isPrintImportModalOpen, setIsPrintImportModal] = useState(false);
    ...
    // // These refs are for the uncontrolled properties (checked, indeterminate)
    // const selectAllHeaderRef = useRef<HTMLInputElement>(null);
    // const selectAllOverlayRef = useRef<HTMLInputElement>(null);
    ...
    // This useEffect correctly sets the `indeterminate` and `checked` state
    // for the uncontrolled input elements using refs.

    const allSelected = currentOrders.length > 0 && selectedOrders.length === currentOrders.length;
    const partiallySelected = selectedOrders.length > 0 && selectedOrders.length < currentOrders.length;
    ...
    const closePrintImportModal = () => setIsPrintImportModal(false);
    ...
    const openPrintImportModal = (e: React.MouseEvent<HTMLButtonElement>) => {
        const rect = e.currentTarget.getBoundingClientRect();
        setModalPos({
            top: rect.bottom + window.scrollY - 55,
            left: rect.left + window.scrollX - 158,
        });
        setIsPrintImportModal(true);
    };
        ...
        <div className="rounded-lg bg-surface-body text-text-main">
            <div className="shadow-xl bg-surface-card rounded-xl">
                    ...
                    <div className="flex flex-col gap-4 mt-6 md:flex-row md:items-center md:gap-4">
                        ...
                        <div className="flex items-center w-full gap-2 md:flex-1">
                            <div className="relative flex-1">
                                ...
                                <input
                                    readOnly
                                    value={selectedCountry ? `Filtering by: ${selectedCountry.name}` : search}
                            ...
                            <div className="text-text-subtle">
                                <button
                                    className="p-2 transition-colors rounded-full hover:text-text-main hover:bg-surface-hover"
                                    onClick={openPrintImportModal}
                                >
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-3">
                                ...
                                <span className="flex items-center px-3 py-2 text-xs font-semibold border border-dashed rounded-lg bg-surface-card border-border-primary text-text-main">
                                    ...
                                    <span className="flex items-center px-2 py-1 ml-2 font-medium rounded-md bg-surface-hover text-xxs text-text-main">
                                        ...
                                        >
                                            <Icon name="x" className="w-1.5 h-1.5 text-white dark:text-black" />
                                        </button>
                                    </span>
                                </span>
                ...
                <div className="relative w-full mt-4">
                                ...
                                <div className="flex items-center gap-4 text-sm font-medium">
                                    <div className="relative w-4 h-4">
                                        ...
                                        <span className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            {allSelected && <Icon name="ri-check-fill" size={12} className="text-white" />}
                                            {partiallySelected && <Icon name="ri-subtract-fill" size={10} className="text-white" />}
                                        </span>
                                    </div>
                    ...
                    <div className="overflow-x-auto overflow-y-visible  max-h-[500px] w-full" style={{ marginTop: selectedOrders.length > 0 ? '0' : '0' }}>
                        <table className="min-w-[800px] md:min-w-full divide-gray-200 w-full">
                            ...
                            <thead className="top-0 z-0 bg-surface-hover text-text-subtle">
                                <tr>
                                    ...
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Order</th>
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Customer</th>
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Date</th>
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Items</th>
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Amount</th>
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Status</th>
                                    <th className="px-3 py-5 text-xs font-semibold text-left">Actions</th>
                                </tr>
                            </thead>
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={clsx(
                                                ...
                                                isChecked ? "bg-surface-hover/50" : "hover:bg-surface-hover"
                                            )}
                                        >
                                            <td className="p-3 w-12 min-w-[48px] pl-6">
                                                <div className="relative w-4 h-4">
                                                    ...
                                                    <span className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                                        {isChecked && (
                                                            <Icon name="ri-check-fill" size={12} className="font-extrabold text-white" />
                                                        )}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="p-3 text-xs font-medium text-text-main">{order.id}</td>
                                            <td className="flex items-center gap-2 p-3">
                                                <img src={order.avatar} alt={order.name} className="w-8 h-8 rounded-full" />
                                                <div>
                                                    <div className="text-xs font-medium text-text-main">{order.name}</div>
                                                    <div className="text-text-subtle text-xxs">{order.email}</div>
                                                </div>
                                            </td>
                                            <td className="p-3 text-xs">
                                                <div className="text-text-main">{order.date}</div>
                                                <div className="text-text-subtle text-xxs">{order.time}</div>
                                            </td>
                                            <td className="p-3 text-xs text-text-main">{order.items}</td>
                                            <td className="p-3 text-xs text-text-main">{order.amount}</td>
                                            <td className="p-3">
                ...
                <div className="flex flex-col gap-4 px-5 py-6 mt-0 text-xs border-t sm:flex-row sm:items-center sm:justify-end sm:gap-8 border-border-primary text-text-subtle">
                    <div className="flex flex-wrap items-center justify-between w-full gap-4 sm:justify-end sm:gap-8 sm:w-auto">
                        ...
                        <span className="font-medium text-text-main whitespace-nowrap">
                            ...
                            {Math.min(indexOfLastRow, filteredOrders.length)} of {filteredOrders.length}
                        </span>
            ...
            {selectedOrderId && (
                <SidebarSubmenu
                    ...
                    onEdit={() => {
                        closePopup();
                        openEditModal(selectedOrderId);
                    }}
                    onDelete={() => { closePopup(); openDeleteModal(); }}
