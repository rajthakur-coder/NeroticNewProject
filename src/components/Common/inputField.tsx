...
interface CustomInputProps {
  ...
  onChange: (value: string) => void;
  leftIcon?: string;
  themeMode?: "light" | "dark"; // ✅ prop to control theme manually
}
...
export default function CustomInput({
  ...
  const handleBlur = () => {
    setIsFocused(false);
    if (inputRef.current && inputRef.current.value === "") setIsAutoFilled(false);
  };
  ...
  const focusBorderColor = isDark ? "#60A5FA" : "#1C252E";
